...
export default async function Users() {
  ...
  return (
    <ChildrenWrapper title={"Users"} description={"Manage and view user details"} LeftComponent={() => <AddUser />}>
      {users.data?.length > 0 ? <UserTable users={users.data} /> : <NotAvailable title={"Users"} />}
    </ChildrenWrapper>
  );
}
